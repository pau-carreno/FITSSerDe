...
/**
 * RecArraySerDe can be used to write data as a sequence of **NumPy** record
 * arrays which, in turn, are the foundation of the **FITS** format.
 * 
 ...
 * STRUCT and UNION.
 * 
 * The mapping between Hive and FITS types is as follows: - BOOLEAN: `L` -
 * TINYINT: `B` (You need a TZERO card to properly store the values) - SMALLINT:
 * `I` - INT: `J` - BIGINT: `K` - CHAR, VARCHAR, STRING: `255A` (Per-column
 * values after HIVE-13064 fix) - DATE: `10A` (ISO-8601: "yyyy-MM-dd") -
 * TIMESTAMP: `23A` (ISO-8601: "yyyy-MM-dd'T'HH:mm:ss.SSS")
 * 
 * NULL values are serialized following FITS standard: - BOOLEAN: `\0` -
 * TINYINT, SMALLINT, INT, BIGINT: `Type.MIN_VALUE` - FLOAT, DOUBLE: `NaN` -
...
public class RecArraySerDe extends AbstractSerDe {
    ...
    public void initialize(Configuration conf, Properties tbl, Properties partitionProperties) throws SerDeException {
        // Always use table properties to initialize
        initialize(conf, tbl);
    }
            ...
            case PRIMITIVE: {
                ...
                default: {
                    throw new SerDeException(
                            "Non supported column primitive type category: " + ptype.getPrimitiveCategory());
                }
                }
                break;
            }
    ...
    public Object deserialize(Writable blob) throws SerDeException {
        ...
        RecordWritable rw = (RecordWritable) blob;

        ArrayList<Writable> recordData = rw.getRecordData();
        ArrayList<String> recordNames = rw.getRecordNames();

        Object[] deserial = new Object[columnNames.size()];
        Object[] rowTemp = new Object[columnNames.size()];
        
        String aux; 
        int j = 0;
        

        // Controls that order in Hive table has not to be the same as in FITS
        for (int i = 0; i < columnNames.size(); i++) {
            j = columnNames.indexOf(recordNames.get(i).toLowerCase()); //Problems with case sensitivity in HIVE
            if (j == -1) {
                throw new SerDeException("Fits " + recordNames.get(i) + " is not found in Hive table");
            }
            rowTemp[j] = recordData.get(i);
        }
        ...
        for (int i = 0; i < columnNames.size(); i++) {
            ...
            case PRIMITIVE: {
                ...
                switch (ptype.getPrimitiveCategory()) {
                case BOOLEAN:
                    deserial[i] = (BooleanWritable) rowTemp[i];
                    break;
                case SHORT:
                    deserial[i] = (ShortWritable) rowTemp[i];
                    break;
                case INT:
                    deserial[i] = (IntWritable) rowTemp[i];
                    break;
                case LONG:
                    deserial[i] = (LongWritable) rowTemp[i];
                    break;
                case FLOAT:
                    deserial[i] = (FloatWritable) rowTemp[i];
                    break;
                case DOUBLE:
                    deserial[i] = (DoubleWritable) rowTemp[i];
                    break;
                case BYTE:
                    deserial[i] = (ByteWritable) rowTemp[i];
                    break;
                case CHAR://TODO implement char /string case
                case VARCHAR:
                case STRING:
                case DATE:
                case TIMESTAMP:
    ...
    public Writable serialize(Object obj, ObjectInspector objInspector) throws SerDeException {
        ...
        for (int i = 0; i < columnNames.size(); i++) {
            ...
            case PRIMITIVE: {
                ...
                case BYTE:
                    if (data != null) {
                        model[i] = new short[] { (short) ((ByteObjectInspector) poi).get(data) };
                    } else {
                ...
                default: {
                    throw new SerDeException(
                            "Non supported column primitive type category: " + poi.getPrimitiveCategory());
                }
                }
                break;
            }
